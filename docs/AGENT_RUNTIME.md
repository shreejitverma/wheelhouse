...
The contract covers automatic PR and issue triage with and without search, the context-equivalent triage correction turn, natural-language schema repair, deep review with and without search, natural-language decision mapping with and without search, and captain-initiated assisted conflict resolution.
...
The other nine actions remain on the exact pinned Claude Action implementation.
...
Before task construction, every spend-capable event creates a durable default-token claim whose key binds the action, target, decision card, exact target revision, and the trigger identity required for deep review and natural-language decisions. Assisted conflict resolution uses that exact target, card, and PR-head identity, allowing only one attempt per head. PR-triage primary and correction claims additionally bind the queue-authorized complete ReviewObservation, base SHA, and default-branch VISION.md SHA-or-trusted-absence through an opaque review-context digest; a checked-in policy recovery also adds its one-use recovery digest. Issue-triage identities remain revision-bound and unchanged.
...
The `merge.resolve-conflicts` action has no write or shell tool: it reads the bounded conflict artifact and returns only the `merge-resolve-v1` per-hunk selection schema, while trusted caller code constructs and confines the resulting bytes.
...
It admits one action family at a time in this order: issue triage, PR triage, schema repair when naturally triggered, deep review, natural-language decisions, then captain-triggered assisted conflict resolution.
